            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.H3(
                            "DASH - EARTHQUAKE DATA APP", style={"color": "SteelBlue"}
                        ),
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        dcc.Loading(
                            id="loading", parent_style=loading_style, type="default"
                        ),
                    ],
                    style={
                        "position": "relative",
...
def display_intensity_plot(evnt_id, sdata):
    ...
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=[
            "CDI Choropleth Mapbox Plot - 1km Spacing",
    ...
    fig.add_trace(
        go.Choroplethmapbox(
            ...
            hovertemplate="UTM Geocode/City: %{text}<br>" + "CDI: %{z}",
            marker=dict(opacity=0.50),
        ),
        row=1,
        col=1,
    )
...
def display_zip_plot(evnt_id, sdata):
    ...
    df = cdi_zip_df
    geo_dff = (
        gpd.GeoDataFrame(sc_zip_df)
        .merge(df, left_on="ZCTA5CE10", right_on="ZIP/Location")
        .set_index("ZIP/Location")
    )
    ...
    fig.add_choroplethmapbox(
        ...
        text=list(df["CDI"]),
        hovertemplate="ZIP/Postal Code:  %{customdata}<br>" + "CDI:  %{text}",
        colorbar=dict(
            orientation="v",
            lenmode="pixels",
            len=435,
    ...
    fig.update_layout(
        ...
        mapbox_zoom=7.5,
        mapbox_center={"lat": 34.0007, "lon": -81.0348},
        ...
        if dataset_df["class"][0] == "scatterplot1":
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name=name,
                    error_y=dict(
                        type="data", array=yerr, color="#778899", visible=True
                    ),
                    mode="markers",
...
def display_dyfi_responses_tbl(evnt_id):
    ...
    fig = go.Figure(
        data=[
            go.Table(
                header=dict(
                    values=list(dyfi_responses_df.columns),
                ...
                cells=dict(
                    values=[
                        dyfi_responses_df["ZIP/Location"],
    ...
    fig.update_layout(
        title_text="Did You Feel It (DYFI) Responses",
        plot_bgcolor="#FAEBD7",
        paper_bgcolor="#FFDEAD",
        autosize=False,
...
def plot_graphs(selectedData, user_input):
    ...
    if selectedData is None:
        ...
        if selectedData["points"][0]["customdata"][6] == 0:
            ...
            return (
                display_intensity_plot(event_id, selectedData),
                new_gp_config,
                new_loading_style,
                False,
            )
            ...
            return (
                display_zip_plot(event_id, selectedData),
                new_gp_config,
                new_loading_style,
                False,
            )
